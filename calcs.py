...
# Optional UI hooks used in this module (warnings/notes). In non-Streamlit contexts,
# this shim makes st.* calls no-ops so imports/tests don’t break.
try:
...
# Fatigue/compliance helpers used below
...
def compute_calcs(
    ...
    # NEW availability constraints (defaults keep existing behaviour)
    route_hours_per_day: float = 24.0,
    route_days_per_year: float = 365.0,
    ms_hours_per_day: float = 24.0,
    ms_days_per_year: float = 365.0,
    ds_hours_per_day: float = 24.0,
    ds_days_per_year: float = 365.0,
):

    trailers_per_set = a_count + b_count
    # Add per-leg compliance buffer (min → h) to each one-way road leg
    # Limit daily driving by both fatigue regime and route operating window
    driver_shift_cap = min(float(driver_shift_cap), max(1.0, float(route_hours_per_day)))

    base_drive_h = distance_km_oneway / max(1e-6, speed_kmh)
    buffer_h = max(0.0, delay_buffer_min) / 60.0
    drive_time_h_oneway = base_drive_h + buffer_h
    ...
    # Add compliance buffer to BOTH driven legs (A→B and B→A). Do NOT add to station bays.
    round_trip_driver_work = (
            (2 * leg_drive) + (2 * leg_breaks) + (2 * buffer_h) + rt_other_work + crew_change_overhead_h
    )
    ...
    if relay_location != "No relay":
        if relay_location == "Camp at A, B & Midpoint":
            # Apply a full buffer per driven half-leg.
            base_drive_h = distance_km_oneway / max(1e-6, speed_kmh)
            buffer_h = max(0.0, delay_buffer_min) / 60.0
            half_leg_drive = (base_drive_h / 2.0) + buffer_h
    ...
    # Incorporate realistic working-day availability
    D_oper = 365.0
    D_driver = float(st.session_state.get("working_days_per_year", 200.0))
    rostered_drivers = int(
        math.ceil(drivers_on_duty * (D_oper / max(1.0, D_driver)))
    )
    ...
    return {
        ...
        "leg_AtoB_work": (
                drive_time_h_oneway + leg_breaks + buffer_h + (unload_time_h if mode == "ThroughRoad" else 0.0)
        ),
        "leg_BtoA_work": (drive_time_h_oneway + leg_breaks + buffer_h),
