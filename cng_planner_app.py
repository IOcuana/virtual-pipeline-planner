...
from physics import density_kg_per_m3, mixture_hhv_J_per_kg, density_mixture_kg_per_m3
...
with st.sidebar:
    ...
    with st.expander("Operating Mode & Demand", expanded=True):
        ...
        project_months = st.number_input(
            ...
            min_value=12, value=60, step=1,
            help="Sets the analysis horizon for roll-ups, finance, and NPV/IRR. "
                 "Minimum 12 months to avoid IRR instability at very short durations."
        )
        st.session_state["project_months"] = float(project_months)  # <-- persist for diagnostics table
        ...
        def rho_at(T_K, P_Pa):
            if gas_for_conv == "Custom CNG mixture":
                mixture_spec = st.session_state.get("cng_mixture_molfractions", {})
                return density_mixture_kg_per_m3(T_K, P_Pa, mixture_spec)
            else:
                if gas_for_conv == "Hydrogen":
                    label = "Hydrogen"
                elif gas_for_conv == "Ammonia (NH₃, compressed)":
                    label = "Ammonia"
                else:
                    label = "Methane"
...
"""
    ...
    with st.expander("🔍 Input-to-Output Diagnostics", expanded=False):
        ...
        # ---- Value lookups for each "Input Label" (read-only; falls back gracefully) ----
        def _fmt(v, unit=""):
            try:
        ...
        def _val(label: str) -> str:
            ss = st.session_state
            try:
                # Sidebar — Operating Mode & Demand / Route / Drivers etc.
                if label == "Mode": return _fmt(ss.get("mode", "DropAndPull"))
                if label == "Project duration [months]": return _fmt(ss.get("project_months"))
                if label == "Operating hours per day [h]": return _fmt(ss.get("hours_per_day"))
                if label == "Daily gas to transport [GJ/day]": return _fmt(ss.get("daily_energy_gj"))
                if label == "Utilisation [% of plan]":
                    frac = float(ss.get("utilisation_frac", 0.0))
                    return f"{frac * 100:,.1f}%"
                if label == "Conversion reference": return _fmt(ss.get("ref_choice", "Standard (15 °C, 1.01325 barₐ)"))
                if label == "Distance Mother→Daughter [km]": return _fmt(ss.get("distance_km_oneway"))
                if label == "Speed [km/h]": return _fmt(ss.get("speed_kmh"))
                if label == "Road Train Combination": return _fmt(ss.get("combo", "A-triple"))
                if label == "Fill rate [GJ/h]": return _fmt(ss.get("fill_rate_gjph"))
                if label == "Concurrent bays per set": return _fmt(ss.get("concurrent_bays_per_set"))
                if label == "Unload time [h]": return _fmt(ss.get("unload_time_h"))
                if label == "Available Mother bays": return _fmt(ss.get("available_bays_A"))
                if label == "Available Daughter bays": return _fmt(ss.get("available_unload_bays_B"))
                if label == "Changeover overhead [h]": return _fmt(ss.get("changeover_overhead_h"))
                if label == "Fatigue regime": return _fmt(ss.get("fatigue_regime", "Standard Hours (Solo Drivers)"))
                if label == "Relay location": return _fmt(ss.get("relay_location", "No relay"))
                if label == "Crew change overhead [h]": return _fmt(ss.get("crew_change_overhead_h"))
                if label == "Delay buffer [min]": return _fmt(ss.get("delay_buffer_min"))
                if label == "Truck utilisation target": return _fmt(ss.get("truck_util_target"))

                # Payload & Capacity
                if label == "Gas Type": return _fmt(ss.get("gas_type_selected", "Methane"))
                if label == "Heating Value basis": return _fmt(ss.get("hv_basis_key", "Gross"))
                if label == "Working Pressure (barₐ)":
                    # stored as bar(g) in key "phys_PW"; convert to bar(a) by +1.01325 for display
                    if "phys_PW" in ss: return _fmt(float(ss["phys_PW"]) + 1.01325)
                    return "—"
                if label == "Min Pressure (barₐ)":
                    # only available when the Payload & Capacity tab has executed; else blank
                    # the key is not persisted; leave "—" if not present
                    return _fmt(ss.get("phys_Pmin_abs_bar")) if "phys_Pmin_abs_bar" in ss else "—"
                if label == "Gas Temperature (°C)": return _fmt(ss.get("phys_T"))

                if label == "Cylinder Volume A-trailer [L]": return _fmt(ss.get("a_cyl_water_L"))
                if label == "Cylinder Volume B-trailer [L]": return _fmt(ss.get("b_cyl_water_L"))
                if label == "Cylinders per A-trailer": return _fmt(ss.get("a_cyl_count"))
                if label == "Cylinders per B-trailer": return _fmt(ss.get("b_cyl_count"))
                if label == "Tare weight A-trailer [kg]": return _fmt(ss.get("tare_a_kg"))
                if label == "Tare weight B-trailer [kg]": return _fmt(ss.get("tare_b_kg"))

                # Infrastructure & Transport Costs
                if label == "Compression plant CAPEX [$]": return _fmt(ss.get("ms_comp"))
                if label == "Utilities CAPEX [$]": return _fmt(ss.get("ms_utils"))
                if label == "Gas conditioning CAPEX [$]": return _fmt(ss.get("ms_gcond"))
                if label == "Filling bay cost per bay [$]": return _fmt(ss.get("ms_bay_cost"))
                if label == "Mother Station OPEX": return _fmt(ss.get("ms_opex_pct_percent")) + " % of CAPEX/yr"
                if label == "Electricity price [$/kWh]": return _fmt(ss.get("ms_elec_price_per_kwh"))
                if label == "Fuel price [$/L]": return _fmt(ss.get("diesel_price_per_l"))
                if label == "Unloading bays cost per bay [$]": return _fmt(ss.get("ds_bay_cost"))
                if label == "Daughter Station OPEX": return _fmt(ss.get("ds_opex_pct_percent")) + " % of CAPEX/yr"

                if label == "Truck CAPEX per unit [$]": return _fmt(ss.get("truck_capex_unit"))
                if label == "A-trailer CAPEX per unit [$]": return _fmt(ss.get("a_trailer_capex_unit"))
                if label == "B-trailer CAPEX per unit [$]": return _fmt(ss.get("b_trailer_capex_unit"))
                if label == "Dolly CAPEX per unit [$]": return _fmt(ss.get("dolly_capex_unit"))
                if label == "Daily CAPEX per unit [$]": return _fmt(ss.get("daily_capex_unit"))

                if label == "Truck maintenance basis": return _fmt(ss.get("maint_basis_trk"))
                if label == "Truck maintenance [$/km or $/year]":
                    return _fmt(ss.get("trk_maint_per_km" if ss.get("maint_basis_trk") == "$/km" else "trk_maint_year"))
                if label == "Trailer maintenance basis": return _fmt(ss.get("maint_basis_trl"))
                if label == "Trailer maintenance [$/km or $/year]":
                    return _fmt(ss.get("trl_maint_per_km" if ss.get("maint_basis_trl") == "$/km" else "trl_maint_year"))

                if label == "Steer Tyres [$/tyre]": return _fmt(ss.get("tyre_steer_price"))
                if label == "Replace at [km/tyre]": return "—"  # ambiguous: multiple rows share this label
                if label == "Drive Tyres [$/tyre]": return _fmt(ss.get("tyre_drive_price"))
                if label == "Trailer Tyres [$/tyre]": return _fmt(ss.get("tyre_trailer_price"))

                if label == "Truck consumption [km/L]": return _fmt(ss.get("truck_km_per_litre"))
                if label == "Diesel price [$/L]": return _fmt(ss.get("diesel_price_per_l"))

                if label == "Wage unit": return _fmt(ss.get("wage_unit"))
                if label == "Wages per driver [$/day or $/year]": return _fmt(ss.get("wage_per_driver"))
                if label == "FIFO costs per driver [$/day]": return _fmt(ss.get("fifo_per_driver_day"))
                if label == "Camp accommodation per driver [$/day]": return _fmt(ss.get("camp_per_driver_day"))
                if label == "Working days per year [days]": return _fmt(ss.get("working_days_per_year"))

                if label == "Annual registration per truck [$/year]": return _fmt(ss.get("reg_truck_per_year"))
                if label == "Annual registration per trailer [$/year]": return _fmt(ss.get("reg_trailer_per_year"))
                if label == "Insurance [$/year]": return _fmt(ss.get("insurance_year"))
                if label == "Miscellaneous [$/year]": return _fmt(ss.get("misc_year"))
                if label == "Finance Plan (rate, NPER)":
                    plans = ss.get("finance_plans", [])
                    enabled = [p for p in plans if p.get("enabled")]
                    if enabled:
                        p = enabled[0]
                        return f'{p.get("label", "")}: {float(p.get("rate_pct", 0)):.2f}% • {int(float(p.get("nper_months", 0)))} mo'
                    return "(no plan enabled)"

                # Benefits & Carbon
                if label == "Revenue - sales gas [$/GJ delivered]": return _fmt(ss.get("sales_gas_value_per_gj"))
                if label == "Baseline": return _fmt(ss.get("baseline_mode"))
                if label == "GWP basis": return _fmt(ss.get("gwp_basis"))
                if label == "Flare efficiency [%]": return _fmt(ss.get("flare_efficiency_pct"))
                if label == "Carbon price [$/tCO₂-e]": return _fmt(ss.get("carbon_price_per_t"))
                if label == "Include transport emissions": return _fmt(ss.get("include_transport_emissions"))
        ...
        # Create DataFrame (+ dynamic Values column)
        import pandas as pd

        df_diag = pd.DataFrame(
            diagnostics_data,
            columns=["Input Label", "Location", "Ribbon Pills", "Diagnostic", "Other Outputs"]
        )
        df_diag["Values"] = [_val(lbl) for lbl in df_diag["Input Label"]]  # <-- NEW
        ...
        # Download button
    csv = df_diag[["Input Label", "Location", "Ribbon Pills", "Diagnostic", "Other Outputs", "Values"]].to_csv(
        index=False)
    st.download_button(
...
with tabs[1]:
    ...
    with colp1:
        with colp1:
            ...
            gas_type = st.selectbox(
                "Gas Type",
                ["Hydrogen", "Methane", "Custom CNG mixture", "Ammonia (NH₃, compressed)"],
                index=1,  # default to Methane
            )
            st.session_state["gas_type_selected"] = gas_type

        # Mixture builder (Option B: expander) – only relevant for Custom CNG mixture
        if gas_type == "Custom CNG mixture":
            with st.expander("Mixture composition (mole fraction)", expanded=False):
                st.caption(
                    "Enter mole fractions for each component. They should add up to about 1.0. "
                    "Inerts (N₂, CO₂) dilute the energy but still add mass."
                )

                # Get previous values or a sensible default CNG-like guess
                default_mix = st.session_state.get(
                    "cng_mixture_molfractions",
                    {
                        "Methane": 0.90,
                        "Ethane": 0.04,
                        "Propane": 0.03,
                ...
                y_ch4 = st.number_input(
                    "Methane (CH₄)", min_value=0.0, max_value=1.0,
                    value=float(default_mix.get("Methane", 0.90)), step=0.01,
                )
                y_c2h6 = st.number_input(
                    "Ethane (C₂H₆)", min_value=0.0, max_value=1.0,
                    value=float(default_mix.get("Ethane", 0.04)), step=0.01,
                )
                y_c3h8 = st.number_input(
                    "Propane (C₃H₈)", min_value=0.0, max_value=1.0,
                    value=float(default_mix.get("Propane", 0.03)), step=0.01,
                )
                y_c4h10 = st.number_input(
                    "Butane (C₄H₁₀)", min_value=0.0, max_value=1.0,
                    value=float(default_mix.get("Butane", 0.01)), step=0.01,
                )
                y_n2 = st.number_input(
                    "Nitrogen (N₂)", min_value=0.0, max_value=1.0,
                    value=float(default_mix.get("Nitrogen", 0.02)), step=0.01,
                )
                y_co2 = st.number_input(
                    "Carbon Dioxide (CO₂)", min_value=0.0, max_value=1.0,
                    value=float(default_mix.get("CO2", 0.00)), step=0.01,
                )

                total_y = y_ch4 + y_c2h6 + y_c3h8 + y_c4h10 + y_n2 + y_co2

                st.session_state["cng_mixture_molfractions"] = {
                    "Methane": y_ch4,
                    "Ethane": y_c2h6,
                    "Propane": y_c3h8,
                    "Butane": y_c4h10,
                ...
                st.markdown(f"**Sum of mole fractions:** {total_y:.3f}")

                if abs(total_y - 1.0) > 0.02:
                    st.warning(
                        "Mole fractions should add up to about 1.0. "
                        "They currently sum to "
                        f"{total_y:.3f}. Adjust if this is not intentional."
                    )
        ...
        DEFAULT_HV = {
            "Methane": {"Gross": 55_571_000.0, "Net": 50_032_000.0},
            "Hydrogen": {"Gross": 141_950_000.0, "Net": 119_910_000.0},
            "Ammonia (NH₃, compressed)": {"Gross": 22_500_000.0, "Net": 18_600_000.0},
        }

        # Compute the "default" HV depending on gas type
        if gas_type == "Custom CNG mixture":
            mixture_spec = st.session_state.get("cng_mixture_molfractions", {})
            hv_default = mixture_hhv_J_per_kg(mixture_spec, basis=hv_key)
        else:
            hv_default = DEFAULT_HV.get(gas_type, DEFAULT_HV["Methane"])[hv_key]
        ...
        # Auto-sync behaviour:
        # - For normal gases: update when Gas Type or Basis changes
        # - For Custom CNG mixture: also update whenever we re-render, so new composition is used
        if auto_sync_hv and (
            gas_type != st.session_state["last_gas_type"]
            or hv_key != st.session_state["last_hv_key"]
            or gas_type == "Custom CNG mixture"
        ):
            st.session_state["HV_J_per_kg"] = hv_default
    ...
    if gas_type == "Custom CNG mixture":
        mixture_spec = st.session_state.get("cng_mixture_molfractions", {})
        rho_full = density_mixture_kg_per_m3(T_K, P_Pa, mixture_spec)
        rho_min = density_mixture_kg_per_m3(T_K, Pmin_Pa, mixture_spec)
    else:
        if gas_type == "Hydrogen":
            gas_label = "Hydrogen"
        elif gas_type == "Ammonia (NH₃, compressed)":
    ...
    HV_J_per_kg = float(st.session_state.get("HV_J_per_kg", 0.0))
...
with tabs[2]:
    ...
    with cT:
        st.markdown(
            "### Ops — Transport "
            "<span title='These values use the planned daily gas (not utilisation-adjusted). "
            "The “Utilisation [% of plan]” input does not change the metrics in this column.'>ⓘ</span>",
            unsafe_allow_html=True,
        )
...
with tabs[3]:
    ...
    # Skip this on the very first render so all tabs show 0.00 initially.
    if not st.session_state.get("app_boot", True):
        try:
            _metrics = compute_levelized_metrics_for_scenario()
...
with tabs[5]:
    ...
    if not first_run:
        ...
        lc_per_gj = lb_per_gj = margin_per_gj = 0.0
    ...
    # Figure out what gas we’re using on this tab
    gas_type_selected = st.session_state.get("gas_type_selected", "Methane")

    # Legacy flag: only Hydrogen follows the old "no methane presets" behaviour
    gas_is_h2 = gas_type_selected == "Hydrogen"

    # New flag: “carbon-free fuels” for messaging (Hydrogen + Ammonia)
    gas_is_carbon_free = gas_type_selected in [
        "Hydrogen",
        "Ammonia (NH₃, compressed)",
    ]
    # (Optional, but handy if other tabs want it:)
    st.session_state["gas_is_carbon_free"] = gas_is_carbon_free
    ...
    with b2:
        ...
        help_flyout(
            "benefits_baseline",
            "Choose the **counterfactual** the project avoids. I...CO₂-e is higher than *Flaring*, which oxidises methane to CO₂.",
        )

        # For hydrogen / ammonia, explain why the numbers look the way they do
        if gas_is_carbon_free:
            st.info(
                "You’ve selected a **carbon-free fuel** (Hydrogen or Ammonia).\n\n"
                "- The **project intensity** is set to 0 tCO₂-e/GJ because the fuel itself "
                "contains no carbon.\n"
                "- The **baseline intensity** and settings below still use **methane venting/"
                "flaring factors** to represent the emissions your project avoids, **not** "
                "ammonia combustion emissions.\n\n"
                "So a reduction of 0.055 tCO₂-e/GJ here means “avoided methane emissions per GJ "
                "of energy delivered”, rather than emissions from burning ammonia."
            )
        ...
        preset_from_url = _qp.get("carbon_preset", "") == "1"
        baseline_from_url = _qp.get("baseline", None)
        gwp_from_url = _qp.get("gwp", None)
        flare_from_url = _qp.get("flare_eff", None)
        vent_qp = _qp.get("vent_ef", None)
        flare_qp = _qp.get("flare_ef", None)
        price_qp = _qp.get("carbon_price", None)
        ...
        with c_row_btn:
            reset_label = (
                "Reset methane presets"
                if not gas_is_carbon_free
                else "Reset baseline methane presets"
            )
            st.button(
                reset_label,
                on_click=_reset_carbon_presets,
                help=(
                    "Apply recommended methane venting/flaring defaults for the baseline. "
                    "For hydrogen or ammonia, these still describe the **methane** you are "
                    "avoiding, not the fuel you’re using."
                ),
            )
...
with tabs[6]:
    ...
    if bool(st.session_state.get("app_boot", True)):
        st.session_state.setdefault("lc_per_gj", 0.0)
        st.session_state.setdefault("lb_per_gj", 0.0)
        st.session_state.setdefault("transport_lc_per_gj", 0.0)
...
# ----- End of first full render (now truly after all tabs) -----
# From the next rerun (any user change), pills will show computed values everywhere.
st.session_state["app_boot"] = False
