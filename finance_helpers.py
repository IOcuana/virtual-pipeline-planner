...
def compute_levelized_metrics_for_scenario(
    ...
    interest_rate_percent = _get_float("interest_rate_percent", 8.0)
    ...
    wage_per_driver = _get_float("wage_per_driver", 600.0)
    fifo_per_driver_day = _get_float("fifo_per_driver_day", 120.0)
    camp_per_driver_day = _get_float("camp_per_driver_day", 150.0)
    working_days = _get_float("working_days_per_year", 365.24)
    drivers_on_duty = int(calcs.get("drivers_on_duty", 0))

    if wage_unit == "$/day":
        driver_cost_year = (wage_per_driver + fifo_per_driver_day + camp_per_driver_day) * drivers_on_duty * working_days
    else:
        driver_cost_year = (wage_per_driver + (fifo_per_driver_day + camp_per_driver_day) * working_days) * drivers_on_duty
